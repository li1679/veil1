...
test('p8 preserves the original iOS color tokens and avoids login overrides', () => {
  const tokens = read('public/css/app/tokens.css');
  const polish = read('public/css/app/visual-polish.css');
  assert.match(tokens, /--accent-blue:\s*#007AFF;/);
  assert.match(tokens, /--system-green:\s*#34C759;/);
  assert.match(tokens, /--system-red:\s*#FF3B30;/);
  assert.doesNotMatch(polish, /--accent-blue\s*:|--accent-blue-active\s*:|--system-red\s*:|--system-green\s*:|--system-yellow\s*:/);
  assert.doesNotMatch(polish, /\.apple-btn|\.btn-primary|\.brand-icon|\.avatar|\.mail-avatar/);
});

test('p8 removes added vivid colors, purple, and the new mailbox frame', () => {
  const polish = read('public/css/app/visual-polish.css');
  assert.doesNotMatch(polish, /violet|purple|#8E5CFF|142,\s*92,\s*255/i);
  assert.doesNotMatch(polish, /#0A84FF|#007AFF|#2F6BFF|#5AC8FA|gradient-brand|brand-cyan|brand-blue|app-accent-muted/i);
...
test('p8 improves shared surfaces, list hierarchy, and state illustrations', () => {
  ...
  assert.doesNotMatch(polish, /\.quota-fill|\.status-dot/);
});
